...
class DataItemStreamWrapper
{
    ...
    /**
     *
     * @var ?\VFramework\App\IDataItemStreamWrapper
     */
    ...
    public function stream_close(): void
    {
        ...
        if ($hasItemSetValueListeners) {
            $dataRepository->dispatchEvent('itemSetValue', new \VFramework\App\Data\ItemSetValueEventDetails($this->key, $value));
        }
        if ($hasItemAppendListeners) {
            $dataRepository->dispatchEvent('itemAppend', new \VFramework\App\Data\ItemAppendEventDetails($this->key, $this->writtenData));
        }
        if ($hasItemGetValueListeners) {
            $dataRepository->dispatchEvent('itemGetValue', new \VFramework\App\Data\ItemGetValueEventDetails($this->key, $value));
        }
        if (($hasItemSetValueListeners || $hasItemAppendListeners) && $dataRepository->hasEventListeners('itemChange')) {
            $dataRepository->dispatchEvent('itemChange', new \VFramework\App\Data\ItemChangeEventDetails($this->key));
        }
        if ($hasItemGetValueListeners && $dataRepository->hasEventListeners('itemRequest')) {
            $dataRepository->dispatchEvent('itemRequest', new \VFramework\App\Data\ItemRequestEventDetails($this->key));
        }
    }
    ...
    public function url_stat(string $path, int $flags)
    {
        ...
        if (substr($path, -3) === '://') {
            ...
            if (array_search($caller, ['filetype', 'is_dir', 'file_exists', 'lstat', 'stat', 'SplFileInfo::getType', 'SplFileInfo::isDir']) !== false) {
                $listContext = new \VFramework\DataList\Context();
                $action = new \VFramework\DataList\FilterByAction();
                $action->property = 'key';
                $action->value = $key . '/';
                $action->operator = 'startWith';
                $listContext->actions[] = $action;
                $action = new \VFramework\DataList\SlicePropertiesAction();
                $action->properties = ['key'];
                $listContext->actions[] = $action;
                $action = new \VFramework\DataList\SliceAction();
                $action->offset = 0;
                $action->limit = 1;
                $listContext->actions[] = $action;
            ...
            if ($dataRepository->hasEventListeners('itemExists')) {
                $dataRepository->dispatchEvent('itemExists', new \VFramework\App\Data\ItemExistsEventDetails($key, $exists));
            }
            if ($dataRepository->hasEventListeners('itemRequest')) {
                $dataRepository->dispatchEvent('itemRequest', new \VFramework\App\Data\ItemRequestEventDetails($key));
            }
            if ($exists) {
                return $result;
            } else {
    ...
    private function getPathInfo(string $path)
    {
        ...
        if (!isset($environment[0]) || !($environment[0] instanceof \VFramework\App\DataRepository)) {
            return false;
        }
        ...
        if (!isset($environment[1]) || !($environment[1] instanceof \VFramework\App\IDataDriver)) {
            return false;
        }
