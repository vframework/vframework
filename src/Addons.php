...
class Addons
{
     ...
     * 
     * @param string $id The id of the addon.
     * @return \VFramework\Addon|null Information about the addon requested or null if not found.
     */
    static function get(string $id): ?\VFramework\Addon
    {
        if (isset(Utilities::$registeredAddons[$id])) {
            $rawData = &Utilities::$registeredAddons[$id];
            if ($rawData[2] === null) {
                $rawData[2] = new \VFramework\Addon($id, $rawData[0], $rawData[1]);
            }
     ...
     * 
     * @return \VFramework\DataList|\VFramework\Addon[] A list of all registered addons.
     */
